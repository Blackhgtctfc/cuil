...
# coding=utf-8
# coding by CUILERON
# Note : jangan di ubah lagi!
# Open source code team | ngotak dikit cok
Hj = '\x1b[1;92m' 
Mt = '\x1b[0m' 
ingfo = (
"""%s
 ...
 - fanspage    : -
 - whatsap     : -
 - github      : github.com/Blackhgtctfc
 - script name : CUIL (Zona Akun Facebook Indonesia)
 - version     : 1.1
...
def masuk():
    ...
    rom = raw_input('\n%s [?] Menu : %s'%(P,K))
    if rom in(""):
    	print("%s [!] Isi yang bena "%(M));exit()
    elif rom in ('1','01'):
        jalan("\n%s [%s!%s]gunakan akun tumbal"%(P,M,P))
        ...
        print (" - siapkan akun facebook (akun tumbal)");jeda(2)
        ...
        if nanya in(""):
        	...
        	print ("\n%s [√] selamat anda goblog :* "%(H));jeda(2);masuk()
        elif nanya in("n","N"):
        	print ("\n%s [!] anda sungguh kontol "%(M));jeda(2);os.system("xdg-open https://youtu.be/IG5QfdxRkeY");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
    	print("%s [!] Isi yang benar "%(M));exit()
